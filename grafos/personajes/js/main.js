...
function setupGUI(config) {
	...
	if (config.logo.file) {
		...
		logo = "<img style=\"width:100%;\" src=\"" + config.logo.file +"\"";
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (2 >= a.length) this.results.html("<i>La búsqueda debe tener un mínimo de 3 letras.</i>");
        else {
            ...
            0 == c.length && !b && a.push("<i>Sin resultados.</i>");
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        var categoriaCluster = sigInst._core.graph.nodesIndex[b[0]].attr.attributes["Categoría"] || "No disponible";
        ...
        sigInst.draw(2, 2, 2, 2);
        $GP.info_name.html("<b> Grupo: <div style=\"width:40px;height:12px;border:1px solid #fff;background:" + a + ";display:inline-block\"></div><br>Categoría: " + categoriaCluster + "</b>");
        $GP.info_data.hide();
